...
52. `SPEC` **UI: Replace the legacy HUD with the modern UI Toolkit match experience**
    ...
    Child #126 makes `TourNumber` and `DeuxCoupsState` authoritative immutable match state. The opening break always establishes Tour 1, later control changes increment the Tour exactly once, and clean continuations plus both bonus opportunities stay inside the same Tour. Wrong-first-ball/no-contact and other non-terminal fouls pass control with `Deux coups`; cue-ball pocket/off-table also grants ball-in-hand; non-8 object balls driven off-table are emitted through `ShotResolution.BallsToRespot`; and assigned-group `Coup mixte` transfers a fresh entitlement. `TwoRemaining` consumes to `OneRemaining` after a clean own-group success or legal miss, opponent-only pockets can terminate the entitlement early, fouls replace the shooter's remaining bonus with the incoming consequence, and after `OneRemaining` is consumed ordinary continuation rules resume. Local validation: full EditMode 384/384 passed and full PlayMode 45/45 passed. Reference: GitHub issue #126 / PR #136.
