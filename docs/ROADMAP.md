...
40G. `PR` **Camera: Move aiming and shot views toward a realistic cue grip position**
    Place the runtime player eye around 60-65% of the cue length behind the cue ball and roughly 18-20 cm above cue-ball height for aiming and shot-power presentation, matching the low close sight-line used by billiards simulations and the user-provided scene reference. The cue butt stays behind the camera so the full cue cannot appear on screen, while the front third remains readable in a 16:10 player view. The runtime camera looks close to the cue ball instead of far down-table; elevation follow still raises the camera with the cue while preserving the table-plane target. Reference: GitHub issue #99 / PR #101.
