...
16. `DONE` **Rules: Implement legal break resolution**
    The pure Core rules layer evaluates the earlier WPA legal-break structure from immutable `ShotFacts`: pocketing any object ball satisfies that diagnostic evaluation, otherwise at least four distinct object balls must reach a rail. This evaluator is retained as compatibility metadata; authoritative Poche libre break progression no longer uses the four-rail threshold. Reference: GitHub issue #32; current break authority: #125.
...
22. `DONE` **Rules: Implement ball-in-hand state**
    Core now models ball-in-hand as immutable match state with an explicit recipient and cue-ball placement area. Standard fouls advance the turn and grant the incoming player placement anywhere. The older restricted above-head-string break helper remains for compatibility, while the authoritative Poche libre break path grants placement anywhere after cue-ball scratch/off-table. Placement consumption clears the state without mutating the previous snapshot, and match invariants reject invalid recipients, phases, or placement scopes. Reference: GitHub issue #44; current break consequence: #125.
23. `DONE` **Rules: Implement called-shot information**
    Core retains the optional object-ball + pocket declaration on `ShotIntent` as compatibility metadata. `ShotFacts` records each pocketed ball together with its stable six-pocket identifier while preserving the derived ball-only list used by existing rules. Poche libre gameplay does not require or expose a declaration, including for the 8-ball. Reference: GitHub issue #46; superseded gameplay authority: #124/#125.
24. `DONE` **Rules: Implement eight-ball win and loss conditions**
    Core resolves terminal 8-ball outcomes into immutable `MatchResult` data with winner, loser, and preserved end reasons. Under the current Poche libre contract, the 8-ball wins in any pocket after the shooter's group is cleared and requires no declaration; pocketing it on the break, pocketing it too early or with a foul, or driving it off-table loses immediately. `ShotFacts` records balls driven off the table, foul resolution reports cue/object-ball off-table faults, and finished match states reject later turn changes. Reference: GitHub issue #48; aligned to current rules by #125.

### Gameplay integration checkpoint

- `DONE` **Gameplay: Add Core rules integration checkpoint**
  `MatchShotResolver` composes immutable shot intent/facts with the Core rules into one authoritative next `MatchState`. Terminal 8-ball outcomes are handled before non-terminal consequences, standard fouls advance the turn with ball-in-hand, Poche libre facts decide group assignment/continuation, and break outcomes resolve directly instead of requiring an external follow-up choice. The resolver remains independent from legacy gameplay singletons and is covered by deterministic EditMode tests plus a scene PlayMode smoke using typed ball identities. Reference: GitHub issue #50; current Poche libre authority: #124/#125.
...
48. `DONE` **Audio: Add rail, pocket and cue impact layers**
...
49. `DONE` **VFX: Add chalk and cue impact feedback**
    Add a presentation-only cue-strike VFX path on top of the typed `CueStrikeApplied` event. The committed shot now exposes its world-space cue-ball contact point, including the selected cue-tip spin offset, and full 3D strike direction for feedback consumers without changing the applied physics. Presentation owns two bounded reusable particle systems: a short blue-white chalk burst and a warmer contact accent, both scaled deterministically from normalized shot power and emitted from the real contact pose. The URP particle shader is referenced explicitly by the gameplay scene so player builds cannot strip it. EditMode covers bounded power scaling and cue-tip contact geometry; PlayMode covers contact placement, resource reuse, scene composition, and the enriched strike observation. Reference: GitHub issue #116 / PR #117.
    Final local validation: repository validation passed, EditMode 348/348 and PlayMode 39/39 passed, and the Windows player build/smoke passed in `Build-Latest` with all startup, local-controls, shot-flow, and ball-in-hand checkpoints. Unity Pipeline Game View captures were also checked at 0.25 and 1.0 normalized power plus a 15-degree elevated strike with side/top spin; the refined burst stays brief and localized at the computed cue-ball contact point without obscuring the ball. Visual evidence is retained under `Build-Latest/VfxProof`.
50. `DONE` **VFX: Add pocket feedback**
    Add presentation-only pocket feedback on top of the existing typed six-pocket capture path without changing capture, rules, or ball physics. Gameplay exposes each pocket's world-space mouth position at playing-surface height, and the impact event source relays a typed capture observation to Presentation. Presentation reuses one bounded cloth-dust ParticleSystem and one bounded rim-accent ParticleSystem, both emitted around the actual captured pocket footprint with no per-capture Instantiate/Destroy churn. EditMode covers the bounded cue, while PlayMode covers mouth localization, repeated resource reuse, cleanup, and full scene composition. Final local validation: repository validation passed, EditMode 349/349 and PlayMode 41/41 passed, and the Windows player build/smoke passed in `Build-Latest`. Unity Pipeline Game View captures were visually checked on both a corner pocket and a side pocket; the refined feedback stays brief, visible around the opening, and localized to the captured pocket. Visual evidence is retained under `Build-Latest/VfxProof`. Reference: GitHub issue #118 / PR #119.
51. `PR` **Camera: Add impact impulse and shot framing**
    Add presentation-only camera impulse on top of the existing typed cue-strike and rail-impact observations, with deterministic thresholds and hard caps so feedback adds weight without changing billiards physics, rules, or input. Extend the modern spectate camera with bounded motion lead and speed-aware pullback while preserving the stabilized cue-relative aiming, shot-power, and ball-in-hand camera ownership. EditMode covers impulse/framing scaling, PlayMode covers scene composition, execution order, bounded runtime impulse, and fast-action spectate framing. Final local validation: repository validation passed, EditMode 354/354 and PlayMode 43/43 passed, and the canonical Windows player build/smoke passed in `Build-Latest`. Unity Pipeline Game View captures were checked for the stabilized aiming view and active spectate framing; visual evidence is retained under `Build-Latest/CameraProof`. Reference: GitHub issue #120 / PR #121.
52. `SPEC` **UI: Replace the legacy HUD with the modern UI Toolkit match experience**
    Replace the existing uGUI/GameObject player-information bar with the complete desktop UI Toolkit flow specified in GitHub issue #122: pre-match names, mirrored player panels, remaining-ball state, active-player emphasis, authoritative `TURN` display, `Deux coups` markers, event feedback, ball-in-hand feedback, and the post-match result screen. The HUD consumes a typed immutable presentation model rather than reading legacy `GameManager`, scene tags, or UI GameObjects directly. Runtime visual validation must use Unity CLI/Pipeline captures at the supported aspect ratios before the step can be considered complete. Reference: GitHub issue #122.
    Delivery uses integration branch `ui/122-ui-toolkit`. The approved child plan is: Poche libre authority #124, break/8-ball outcomes #125, Tour/Deux coups #126, UI Toolkit match shell #127, mirrored player panels #128, ball/8-ball HUD state #129, TURN/events #130, result flow/statistics #131, responsive/presentation polish #132, and final integrated validation #133. Child pull requests target the integration branch; only the completed integration branch is eventually reviewed for merge to `main`.
    Child #124 implements authoritative `Poche libre` resolution from observed shot facts: a single pocketed family assigns an open table and keeps control, mixed families keep an open table and pass the Tour, clean own-group pockets keep control, opponent-only and mixed-group pockets pass the Tour without creating a foul, and legal no-pocket shots pass the Tour cleanly. Legacy called-shot data remains compatibility metadata only and no longer controls non-terminal normal Tour ownership, group assignment, or whether an otherwise valid non-terminal normal shot can be resolved. Break/8-ball alignment remains owned by child #125. Local validation: repository validation passed, targeted `MatchShotResolverTests` 27/27 passed, full EditMode 364/364 passed, and full PlayMode 45/45 passed. Reference: GitHub issue #124 / PR #134.
    Child #125 aligns break and terminal 8-ball outcomes with `Poche libre`: a clean break pocketing one family assigns it immediately and keeps control; mixed families keep the table open and pass control; a break cue-ball scratch preserves any single-family assignment, grants the opponent ball-in-hand anywhere, and signals a `Deux coups` entitlement for #126 to persist. The 8-ball now loses immediately on the break, when pocketed too early or with a foul, or when driven off-table; after the shooter's group is cleared it wins in any pocket without declaration. Terminal 8-ball resolution runs before break consequences. Local validation: repository validation passed, targeted `EightBallRuleTests` 14/14 passed, targeted `MatchShotResolverTests` 32/32 passed, full EditMode 369/369 passed, and full PlayMode 45/45 passed. Reference: GitHub issue #125 / PR #135.
    Child #126 makes `TourNumber` and `DeuxCoupsState` authoritative immutable match state. The opening break always establishes Tour 1, later control changes increment the Tour exactly once, and clean continuations plus both bonus opportunities stay inside the same Tour. Wrong-first-ball/no-contact and other non-terminal fouls pass control with `Deux coups`; this includes a break with no object-ball contact. Cue-ball pocket/off-table also grants ball-in-hand; non-8 object balls driven off-table are emitted through `ShotResolution.BallsToRespot`; and assigned-group `Coup mixte` transfers a fresh entitlement. `TwoRemaining` consumes to `OneRemaining` after a clean own-group success or legal miss, opponent-only pockets can terminate the entitlement early, open-table mixed-family outcomes end the entitlement and pass normally without creating a new bonus, fouls replace the shooter's remaining bonus with the incoming consequence, and after `OneRemaining` is consumed ordinary continuation rules resume. Local validation: full EditMode 386/386 passed and full PlayMode 45/45 passed. Reference: GitHub issue #126 / PR #136.
    Child #127 replaces the active legacy bottom HUD shell with the first UI Toolkit match flow: pre-match player-name setup, 12-character input rules and fallbacks, keyboard/gamepad focus navigation, 50/50 breaker selection, one typed immutable presentation model, authoritative active-player/Tour projection, and an inactive legacy uGUI root. The real scene now loads the `UIDocument` setup before gameplay is enabled, then hands control to the existing gameplay state machine only after START. Local validation: repository validation passed, full EditMode 394/394 passed, full PlayMode 48/48 passed, and the canonical Windows player build/smoke passed in `Build-Latest`. Unity Pipeline Game View captures were checked at 1280x720, 1920x1080, and 2560x1080 for both setup and in-match HUD states. Reference: GitHub issue #127 / PR #137.
53. `REMOVED` **UI: Add called-shot interaction**
    Removed from the game design. The ruleset uses `Poche libre`: no ball/pocket declaration is required, including for the 8-ball. Existing called-shot code may remain temporarily as internal technical debt only if it does not affect the agreed rules; the UI must not expose or depend on called-shot interaction. Superseded by GitHub issue #122.
54. `SPEC` **UI: Add turn, foul, ball-in-hand, and result feedback**
    Consolidated into the UI Toolkit specification in GitHub issue #122 so turn ownership, event timing, foul consequences, `Deux coups`, ball-in-hand, victory/defeat presentation, and navigation are implemented against one presentation contract instead of separate legacy HUD paths. Reference: GitHub issue #122.
