...
40C. `PR` **Testing: Add Windows player smoke and functional validation**
    Validate the built player rather than only the Editor, including startup, scene loading, real local Input System input, shot flow, ball-in-hand, and clean shutdown with machine-readable evidence. The Windows x64 development-player smoke now requires all 16 balls to be active, observes the scene aiming/spin controllers consuming injected Input System state, commits a shot through the real play/shoot controller path and verifies cue-ball movement, captures a scratch through `BallPocketCapture` before restoring it through legacy ball-in-hand flow, preserves the post-placement input-release guard, and fails on runtime player errors. Local validation passes with the Windows smoke, EditMode 300/300, PlayMode 26/26, repository validation, and `git diff --check`. Reference: GitHub issue #92 / PR #93.
