...
40G. `PR` **Camera: Move aiming and shot views toward a realistic cue grip position**
    Move the runtime player eye closer to the cue for both aiming and shot-power presentation while keeping it behind the readable grip area instead of intersecting the cue. The final framing keeps roughly the front half of the cue inside a 16:10 player view, while elevation follow raises the camera with the cue and the forward look target stays on the playable table plane. This prevents elevated aim from pitching the view below the table or pushing the cue out of frame. Reference: GitHub issue #99 / PR #101.
