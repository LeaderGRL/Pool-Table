...
51. `DONE` **Camera: Add impact impulse and shot framing**
...
52. `DONE` **UI: Build modern match HUD**
    Replace the legacy bottom player-information bar with a responsive top match HUD using the supplied black/gold visual direction. Normalize the supplied runtime assets under `Assets/UI`, derive the right player panel from the corrected left source so both sides remain geometrically coherent, and keep the supplied full-layout mockup under `references`. The HUD shows both player labels, active-turn emphasis, remaining numbered balls from typed capture state, and a centered single-number round display without conflating legacy turn count with match rounds. Called-shot interaction and foul messaging remain in steps 53 and 54. Final local validation: repository validation passed, EditMode 358/358 and PlayMode 48/48 passed, and the canonical Windows player build/smoke passed in `Build-Latest` with startup, Input System, local controls, shot-state, spectate-state, return-to-play, shot-flow, ball-in-hand, and clean-shutdown checkpoints. Unity Pipeline Game View captures were visually checked at 1920x1080, 1600x1000, and 2560x1080; the HUD remains symmetric, centered, readable, and free of overlap while the legacy bottom bar stays hidden. Visual evidence is retained under `Build-Latest/HudProof`. Reference: GitHub issue #122.
53. `ACTIVE` **UI: Add called-shot interaction**
...
Current Phase 7 work is **UI: Add called-shot interaction** (step 53). Modern match HUD work is complete on issue #122 and awaiting merge through its implementation PR, camera impact impulse and shot framing are merged through PR #121, pocket feedback is merged through PR #119, URP conversion is merged through the platform quality profiles in PR #111, impact audio is merged through PR #115, cue-impact VFX is merged through PR #117, and stabilization steps 40G–40I are merged through PR #101 and manually validated. Before every new issue, review this roadmap and verify the current status from repository, test, or merged-PR evidence.
