...
45. `PR` **Rendering: Add post-processing quality profile**
    Add a dedicated global URP Volume for the pool-table scene with ACES tonemapping, restrained color adjustments, subtle bloom, and a light vignette. Post-processing is enabled explicitly on the main camera and covered by EditMode checks for both scene wiring and profile values. Local validation: repository validation passed, EditMode 320/320, PlayMode 34/34, Windows player smoke passed, and a D3D11 player capture was visually checked. Keep Windows/Web-specific quality scaling in step 46. Reference: GitHub issue #108.
