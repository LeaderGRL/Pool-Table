...
Rolling resistance updates both planar linear velocity and the matching horizontal angular velocity so the no-slip condition remains coherent as the ball slows. Vertical linear velocity remains owned by PhysX, while vertical-axis spin is preserved for the dedicated spin work that follows. Billiard rigidbodies allow angular velocity up to 200 rad/s because a regulation-radius ball already requires about 70 rad/s to roll at 2 m/s; Unity's much lower default cap would otherwise invalidate the transition model. Cloth resistance is still applied only when an upward supporting contact comes from a collider explicitly marked with `ClothSurface`, so ball-to-ball, rail, and pocket contacts cannot accidentally receive cloth resistance. The sliding and rolling values are initial calibration baselines for later instrumentation. Cue-spin controls, spin decay, rail restitution, and pocket capture remain separate follow-up systems.
