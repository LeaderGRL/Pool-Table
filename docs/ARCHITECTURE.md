...
`BilliardsSimulationConfiguration` is the authoritative rigidbody/simulation baseline layered on those dimensions. Ball rigidbodies use a 0.17 kg mass, zero built-in linear/angular damping, continuous dynamic collision detection, and interpolation. The project physics baseline uses a 1 mm default contact offset, a lower sleep threshold, 12/4 solver iterations, and a 0.005 s fixed timestep. The legacy cue controller expresses shot strength as a target velocity change in meters per second, so changing Rigidbody mass does not silently retune shot speed. Cloth resistance, rolling transition, spin, rail restitution, and pocket capture remain explicit follow-up systems rather than being hidden in generic Rigidbody damping.
