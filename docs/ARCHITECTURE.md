...
`BallClothResistance` now owns the explicit rolling-resistance baseline. A supported ball loses 0.2 m/s of planar speed per second, independently of Rigidbody mass and fixed-step size, while vertical velocity remains under PhysX control. The model clamps low speeds to zero instead of reversing velocity and only applies when an upward supporting contact comes from a collider explicitly marked with `ClothSurface`, so ball-to-ball, rail, and pocket contacts cannot accidentally receive cloth resistance. The tabletop uses that marker together with a dedicated zero-friction PhysicMaterial so native PhysX friction cannot add hidden deceleration or sliding-to-rolling coupling on top of this explicit model. The 0.2 m/s² value is an initial calibration baseline for later instrumentation rather than a hidden PhysX damping value. Sliding-to-rolling coupling, spin, rail restitution, and pocket capture remain separate follow-up systems.
