...
The product break contract has no minimum power, spread, or rail-count requirement. Contact-foul and `Deux coups` lifecycle semantics are handled by authoritative match state rather than by `LegalBreakRule`.
...
`MatchState` also owns the authoritative `TourNumber` and `DeuxCoupsState`. The break itself is Tour 0; leaving the opening break establishes Tour 1 whether the breaker keeps control or opening control passes. Later player changes advance the Tour exactly once, while clean continuations and both bonus opportunities remain inside the same Tour. `DeuxCoupsState` is explicit (`Inactive`, `TwoRemaining`, `OneRemaining`) and always belongs implicitly to `CurrentPlayer`, so presentation does not need a second recipient field to reconcile.
...
During the break this resolver reports directly observable scratch/off-table fouls and the product-specific no-object-contact foul without applying the normal-shot first-contact or rail/pocket rules. Gameplay composes those facts with the Poche libre break result and the authoritative Tour/Deux coups lifecycle. The physics observation layer is responsible for producing the corresponding `ShotFacts`; the Core only evaluates those facts.

`BallInHandRule` applies the ball-in-hand consequence without introducing Unity physics concerns. Under the authoritative Poche libre resolver, cue-ball pocket/off-table fouls pass control, grant the incoming player placement anywhere, and combine that state with `Deux coups`; wrong-first-ball, no-contact, rail/pocket, and object-ball-off-table fouls pass control with `Deux coups` without adding ball-in-hand. The authoritative break path also grants placement anywhere after a cue-ball scratch/off-table foul, after any single-family assignment has been preserved. The older above-head-string break helper remains only for compatibility with earlier rule slices and is not used by `MatchShotResolver`. Once gameplay confirms a valid cue-ball placement, the rule consumes the ball-in-hand state while preserving the same Tour and match phase. Physical placement coordinates, overlap checks, and table-boundary validation remain gameplay/physics responsibilities.
...
The resolver owns rule ordering rather than reimplementing individual Core rules. It resolves terminal 8-ball outcomes first, so an 8-ball loss on the break takes precedence over group assignment, scratch, ball-in-hand, or any other non-terminal consequence. For non-terminal normal shots it evaluates the composed foul result, passes control, grants a fresh `Deux coups`, and adds ball-in-hand only for cue-ball pocket/off-table fouls. Clean `Poche libre` progression is derived from `ShotFacts`: on an open table, pocketing exactly one family assigns that family and keeps control, while pocketing both families keeps the table open and ends the Tour. After groups are assigned, pocketing only the shooter's group keeps control; opponent-only pockets and legal no-pocket shots normally end the Tour, while a `Coup mixte` transfers control with a fresh `Deux coups`. During the first bonus opportunity, a clean own-group success or legal miss consumes one opportunity but preserves control; opponent-only pockets end the entitlement and pass normally; an assigned-group `Coup mixte` transfers a fresh entitlement; an open-table mixed-family outcome ends the entitlement and passes control normally; and fouls discard the shooter's remaining entitlement before applying the incoming foul consequence. The second bonus opportunity is consumed before ordinary continuation rules resume. Legacy called-shot success may still be reported as compatibility metadata, but it does not control normal or terminal Poche libre outcomes. Input validation still rejects observed contacts, pockets, and off-table object balls that were not present in the pre-shot snapshot, which makes the boundary suitable for future local and network adapters.

Break shots resolve directly. A clean single-family pocket assigns that family and keeps the breaker at the table; mixed families keep the table open and pass opening control; a clean no-pocket break opens the table and passes opening control. A break with no object-ball contact is a foul that passes opening control and grants `DeuxCoupsState.TwoRemaining`. Every non-terminal break result enters Tour 1. A cue-ball scratch/off-table foul preserves any single-family assignment, passes opening control, grants ball-in-hand anywhere, and persists `DeuxCoupsState.TwoRemaining` for the incoming player. Non-8 object balls driven off the table are exposed through `ShotResolution.BallsToRespot` and also grant the incoming player `Deux coups`; the 8-ball remains terminal and is never respotted. `ShotResolution.GrantsTwoShotEntitlement` is retained only as a derived compatibility view of the typed match state. `BreakEvaluation` remains diagnostic compatibility metadata and `RequiresBreakFollowUp` is false for authoritative break resolutions.
