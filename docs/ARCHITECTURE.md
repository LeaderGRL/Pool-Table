...
`BilliardsSimulationConfiguration` is the authoritative rigidbody/simulation baseline layered on those dimensions. Ball rigidbodies use a 0.17 kg mass, continuous dynamic collision detection, and interpolation. Until the dedicated cloth model replaces it, the scene deliberately retains the legacy 0.35 linear / 0.2 angular Rigidbody damping as a temporary turn-termination safeguard so rolling balls cannot keep gameplay in spectate indefinitely. The project physics baseline uses a 1 mm default contact offset, a lower sleep threshold, 12/4 solver iterations, and a 0.005 s fixed timestep. Legacy upward-velocity damping is expressed as a time-scaled retention factor so the move from the old 0.02 s timestep to 0.005 s preserves the same damping over a 20 ms interval instead of applying the old multiplier four times. The legacy cue controller expresses shot strength as a target velocity change in meters per second, so changing Rigidbody mass does not silently retune shot speed. Cloth resistance, rolling transition, spin, rail restitution, and pocket capture remain explicit follow-up systems; the next cloth-resistance issue must remove this temporary damping when the replacement resistance model is connected.
